...
code40 <- .eng[, .(cod = mean(oc)), by = responseid][cod <= 0.40, responseid]
n0 <- nrow(firm_long_dt)
firm_long_dt <- firm_long_dt[!(responseid %in% code40)]
if (exists("resp_dt")) resp_dt <- resp_dt[!(responseid %in% code40)]
cat("Dropped", length(code40), "code 0-40% Engineer evaluators | rows", n0, "->", nrow(firm_long_dt), "\n")

source(here("Temp", "_6_relaxed.R"))

today_str <- format(Sys.time(), "%Y%m%d_%H%M")
out <- sprintf("SideReport_DropCode40Eng_%s.pdf", today_str)
