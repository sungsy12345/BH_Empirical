...
lowcode <- .eng[, .(cod = mean(oc)), by = responseid][cod <= 0.20, responseid]
n0 <- nrow(firm_long_dt)
firm_long_dt <- firm_long_dt[!(responseid %in% lowcode)]
if (exists("resp_dt")) resp_dt <- resp_dt[!(responseid %in% lowcode)]
cat("Dropped", length(lowcode), "low-code (0-20%) Engineer evaluators | rows", n0, "->", nrow(firm_long_dt), "\n")

source(here("Temp", "_6_relaxed.R"))

today_str <- format(Sys.time(), "%Y%m%d_%H%M")
out <- sprintf("SideReport_DropLowCodeEng_%s.pdf", today_str)
